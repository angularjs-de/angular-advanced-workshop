...
describe('BookEditComponent', () => {
  ...
  it('should display the isbn of the loaded book', () => {
    expect(compiled.querySelector('input[name="isbn"]').value).toContain(
      '978-0-20163-361-0'
    );
    expect(compiled.querySelector('input[name="title"]').value).toContain(
      'esign Patterns'
    );
    expect(compiled.querySelector('input[name="author"]').value).toContain(
  ...
  it('should validate that title is required', fakeAsync(() => {
    ...
    expect(compiled.querySelector('.title-error').innerText).toContain(
      'Title is required'
    );
  }));
